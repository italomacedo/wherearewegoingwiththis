...
**Phases 0–8 COMPLETE** (+ gaps #1/#2/#3 closed) · **Phase 9 MVP** (flying motorcycle) · **pause/save + HUD + camera-relative WASD + Z/C & MMB orbit** · **health system (fall damage, bike crash → smoke → explode, game-over, persisted)** · **playable MVP verified in Electron (live Claude NPC chat)** · **Avatar overhaul — [ADR-0014](docs/ADR/0014-avatar-pipeline-makehuman-morphs.md) (+ addendum): real MakeHuman/MPFB2 GLB hero loads in Electron (`useGltf=true`); creator does GENDER + ETHNICITY by file (`body_<gender>_<african|asian|caucasian|universal>`) + SKIN-TONE tint on the real material. MPFB exports no fine shape keys (+ Apply-Modifiers strips macro morphs) → runtime morph sliders dropped (plumbing dormant); ethnicity = one GLB per ethnicity. **RIG + ANIMATION DONE (Addendum 2): hero rigged via Mixamo (FBX upload → auto-rig → re-download with skin = new base; all `mixamorig:` bones) + 4 shared clips (`characters/animations/{idle,walk,run,interact}.glb`) retargeted onto the base skeleton at runtime + renamed for name-based playback; Mixamo faces +Z so the 180° flip was REMOVED (moonwalk fix) and the creator camera moved to `alpha=+π/2`. `scripts/convert_anims.py` batch-converts FBX→GLB headless.** Flying bike also loads a real GLB (`vehicles/cyberpunk_harley.glb`) via the same guarded-load/fallback. Deferred: hair/clothing GLBs (next), skin PNGs, distinct per-ethnicity/gender rigged exports (8 bases are copies of the Mixamo male-caucasian); hero mesh disappears while mounted (separate task)** · 620 tests · ~99% coverage (gated 95% lines/stmts/funcs, 90% branches) · typecheck + build green.
