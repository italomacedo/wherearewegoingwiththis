...
| 13 (audio) | **Game audio — 13A/B/C COMPLETE & owner-validated; 13D (Voice/TTS) pending** (branch `feat/audio-system`, NOT merged). 13A: `AudioManager` (pure bus mixer master/music/sfx/voice + HTMLAudioElement playback) + **Sound options tab** (volumes/mutes/TTS toggle) reading the existing `SettingsService` volume fields. 13B: `SfxCatalog` + EventBus/seam SFX (footsteps, combat beats swing/stab/punch/whiff/gunshot/bodyfall, eat, hunger growl, nave explosion, UI click/open/error reusing existing cues; procedural Web-Audio engine drone). 13C: `MusicCatalog` per-scene beds + crossfade (opening theme on splash/studio/publisher, menu, world street ambience, combat, game-over). CC0/CC-BY freesound clips via `scripts/convert_audio.mjs` (ffmpeg-static), CC-BY credited in `CREDITS.md`. [ADR-0025](docs/ADR/0025-audio-and-tts.md) | 🟡 |
