...
Next (unstarted, owner's call — **to be replanned**): a **2nd street** via the +X exit wall (scene transition); **weapons/ammo + inventory** (re-enables Shoot/Reload + diegetic corpse frisk/loot); **per-NPC stat blocks**; real Claude tokens (`--output-format json`); more NPCs/routines; scenery cover; implants. (Combat multi-combatente já entregue na Fase 8.)
