...
| 12 (modular avatars) | **Modular avatars — CODE COMPLETE on `feat/modular-avatars` (awaiting Electron playtest)**: combine **head + top + bottom** from different Quaternius molds, rebinding region meshes to one shared skeleton by bone index (identical joint order within a gender); **top/bottom clothing recolor** (uniform per region) + themed-mold hair override (punk mohawk); **weapon meshes stripped** (Pistol/Sword — also fixes Suit/SWAT pistol-in-hand); creator gains 3 part cyclers + Top/Bottom Color. `avatarPieces` repurposed; empty map = legacy whole-outfit look. [ADR-0024](docs/ADR/0024-modular-avatars.md) | 🟡 |
