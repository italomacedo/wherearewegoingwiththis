...
Next (unstarted, owner's call — **to be replanned**): a **2nd street** via the +X exit wall (scene transition); **firearms/ammo + Shoot/Reload** (re-enables ranged combat); **per-NPC stat blocks**; real Claude tokens (`--output-format json`); more NPCs/routines; scenery cover; implants; Zara shop/economy. (Combat multi-combatente entregue na Fase 8; inventário + loot de cadáver na Fase 9.)
...
| 11+ | Implants, world expansion (next scene via the +X exit wall), **firearms/ammo + Shoot/Reload**, scenery cover, per-NPC stat blocks, Zara shop/economy | ⬜ |
