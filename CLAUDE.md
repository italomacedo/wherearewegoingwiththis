...
> **Combat track — Fase 7 (turn-based combat MVP) — [ADR-0019](docs/ADR/0019-turn-based-combat.md). On branch `feat/combat` (not yet merged). 918 tests, ~98%/91% coverage, typecheck+build green.** Consumes the `attack` intent stub from F5. **Model (owner-decided):** a turn ≈ 1 s; **`AP = round(Destreza/10)`** (min Dex 20 = 2 AP = one primary action only; Dex 60 = 6; 100 = 10); costs **primary 2 / secondary 1 / movement 1 AP per metre**, no cap on primaries (AP is the only limit); **scalar distance in metres** (no grid), melee needs ≤1.5 m; **hit via power-ratio `SkillCheck`** (melee=Combate C-a-C/Força, ranged=Armas de Fogo/Destreza vs Percepção + cover +20/+40); damage attribute-scaled + variance; Destreza initiative; **1v1, only a hostile NPC triggers it**. **All tunable in Options** (`combatApPerDexterity`/`combatPrimaryCost`/`combatSecondaryCost`/`combatMoveApPerMeter`). **Pure, 100%-covered core** in `src/systems/combat/`: `CombatMath` (AP/move/hit/damage/initiative/tuning), `CombatEncounter` (state machine: AP refill, distance, cover resets per turn + breaks on move, win/lose/flee), `CombatAI` (`chooseCombatAction`: cover-when-hurt / brawler-closes-then-strikes / gunner-shoots-then-covers), `CombatNarration.combatBeat` (factual line = Claude seed + offline fallback), `CombatController` (player→enemy turn orchestration + `playerActionOptions`). **Browser-only (`istanbul ignore`d):** `CombatOverlay` (AP/distance/HP readout, scrolling beat log, action buttons; Claude dramatizes salient beats via `NPCManager.narrateCombat`, bounded cost) + `GameWorldScene` wiring (attack intent → `startCombat`; overlay freezes the world like pause; outcome syncs player HP, relaxes a **defeated** NPC's disposition, loss → game-over). **Verified in Electron** (provoke → attack → dodge → end). Fixes from playtest: hostile player actions were never wired to disposition → added a context-aware **`HOSTILE`** flag to the action classifier so striking an NPC worsens its disposition (1st blow = ultimatum, 2nd = hostile → duel starts); enemy-first initiative deadlock fixed by pumping the enemy turn on open; action bar widened so all buttons fit.
>
> **Fase 7.2 — Baldur's-Gate presentation (same branch, green):** narration **only on a critical hit** (P>0.9, `isCriticalHit`) shown as a **transient caption** (chat-log box removed); **real avatar animations** from the Quaternius embedded clips (no UAL/retarget) — `COMBAT_CLIPS` punch/kick/shoot/aim + **HitRecieve/Death**, kept by `assembleGltf`, played one-shot via `GameWorldScene.playCombatClip` + the `CombatOverlay.onBeat` handler (muzzle-flash deferred); **cinematic combat camera** framing the two fighters; **3D portrait strip** (`CombatPortraits`, browser-only) = per-combatant head-framing viewport camera in initiative order + neon turn marker. **Melee combat phase DONE** (owner-validated in Electron): melee-only loadout (`CombatCapabilities`/`MELEE_ONLY_CAPS` — no Shoot/Reload/Cover/Hunker yet, NPC forced to melee), lunge-punch-retreat choreography + HitRecieve/Death clips, right-side objective log (roll vs chance), **rare** critical (d100<5) → short poetic line, chat closes on combat start, and a **Game Over menu** (Load Last Save / Return to Main Menu; death clip held; autosave skipped on death). **Deferred:** multi-combatant / faction fights (allies & enemies joining by NPC↔NPC disposition — owner's chosen next combat step), **block/crouch animations (need a rig retarget)**, weapons/ammo + inventory (re-enables Shoot/Reload), scenery cover (re-enables Cover/Hunker), per-NPC stat blocks, mid-fight save, muzzle-flash, in-turn animation sequencing.

Next (unstarted, owner's call): playtest Fase 7.2 presentation in Electron + merge `feat/combat`; **multi-combatant/faction combat**; a **2nd street** via the +X exit wall; real Claude tokens (`--output-format json`); more NPCs/routines.
...
| 10 | **Turn-based combat — MVP** (AP from Destreza, power-ratio hit, scalar distance, hostile-NPC trigger; [ADR-0019](docs/ADR/0019-turn-based-combat.md)) | ✅ MVP |
| 11+ | Implants, world expansion (next scene via the +X exit wall), multi-combatant + weapons | ⬜ |
