...
Next (unstarted, owner's call): **multi-combatant/faction combat**; a **2nd street** via the +X exit wall; real Claude tokens (`--output-format json`); more NPCs/routines.
...
| 8 (tactical) | **Tactical combat — CODE COMPLETE on `feat/combat-tactical`** (spatial Move/AP routed around obstacles + on-ground trail; melee ≤1 m; flee >10 m; **multi-combatant by relationship ledger, no factions** — recruited sides, allies AI-fight, player-flee-continues, friendly-fire defection, autonomous/spectator NPC↔NPC fights; [ADR-0020](docs/ADR/0020-tactical-multicombatant-combat.md)) | 🟡 awaiting Electron playtest |
| 11+ | Implants, world expansion (next scene via the +X exit wall), weapons/ammo + inventory, scenery cover, per-NPC stat blocks | ⬜ |
